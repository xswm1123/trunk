...
{
    if (error) {
        [PYToast showWithText:@"图片保存失败！"];
//        [MBProgressHUD showSuccess:@"保存失败" toView:[UIApplication sharedApplication].keyWindow];
        NSLog(@"失败");
    } else {
        MJPhoto *photo = _photos[_currentPhotoIndex];
        photo.save = YES;
        ...
        NSLog(@"成功");
         [PYToast showWithText:@"图片成功保存到相册！"];
//        [MBProgressHUD showSuccess:@"成功保存到相册" toView:[UIApplication sharedApplication].keyWindow];
    }
}
