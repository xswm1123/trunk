...
@interface AdaptLeveyPopListView (private)
- (void)fadeIn;
- (void)fadeOut;
@end

@implementation AdaptLeveyPopListView {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([_delegate respondsToSelector:@selector(AdaptLeveyPopListView:didSelectedIndex:)])
        [_delegate AdaptLeveyPopListView:self didSelectedIndex:[indexPath row]];
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if ([_delegate respondsToSelector:@selector(AdaptLeveyPopListViewDidCancel)])
        [_delegate AdaptLeveyPopListViewDidCancel];
...
- (void)drawRect:(CGRect)rect
{
    ...
    CGRect titleRect = CGRectMake(POPLISTVIEW_SCREENINSET + 10+5, POPLISTVIEW_SCREENINSET + 10 + 5,
